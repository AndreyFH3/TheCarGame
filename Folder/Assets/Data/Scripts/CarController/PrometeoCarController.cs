...
public class PrometeoCarController : MonoBehaviour
{
    ...
    */
    ...
    public System.Action<float> OnCarSpeedChage; // Used to store the UI object that is going to show the speed of the car.
    ...
    */
    ...
    void Update()
    {
        ...
        */
        if (useTouchControls && touchControlsSetup) {
            ...
            if (!handbrakePTI.buttonPressed || Input.GetKeyUp(KeyCode.Space)) {
                RecoverTraction();
            }
        ...
        OnCarSpeedChage?.Invoke(Mathf.RoundToInt(absoluteCarSpeed));
    ...
    public void Init(CarSettings settings, bool isDesktop)
    {
        if (isDesktop)
        {
            ...
            InitTouchControll(instance);
        }

        maxSpeed = settings.MaxSpeed;
        maxReverseSpeed = settings.MaxReverseSpeed;
        accelerationMultiplier = settings.AccelerationMultiplayer;
        maxSteeringAngle = settings.MaxSteeringAngle;
        steeringSpeed = settings.SteeringSpeed;
        brakeForce = settings.BrakeForce;
        decelerationMultiplier = settings.DecelerationMultiplayer;
        handbrakeDriftMultiplier = settings.DriftMultiplayer;
    ...
    private void InitTouchControll(TouchInputView input)
    {
        throttleButton = input.Forward;
        reverseButton = input.Back;
        turnRightButton = input.Right;
        turnLeftButton = input.Left;
        handbrakeButton = input.Brakes;
    }
